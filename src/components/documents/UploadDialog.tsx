...
export function UploadDialog({ isOpen, onClose, onUpload }: UploadDialogProps) {
  ...
  const categories = [
    ...
    { value: "LAB_TEST_REPORT", label: "Lab Test Reports (Blood, Urine, etc.)" },
  ...
  const getPlaceholderForCategory = (category: string) => {
    const placeholders: Record<string, string> = {
      ...
      LAB_TEST_REPORT: "e.g., Complete blood count, glucose test, cholesterol panel...",
      PRESCRIPTION: "e.g., Medication name, dosage, prescribing doctor...",
      REFERRAL_LETTER: "e.g., Referral to cardiologist, specialist consultation...",
      SURGERY_RECORD: "e.g., Appendectomy, procedure notes, recovery details...",
      VACCINATION_CERTIFICATE: "e.g., COVID-19 vaccine, flu shot, travel vaccines...",
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200 dark:border-slate-700 w-full max-w-md max-h-[90vh] my-8 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1">
          ...
          <div className="mt-3 flex items-start space-x-2 text-sm text-amber-600 dark:text-amber-400">
            ...
            <span>Maximum file size: 10MB. Supported formats: PDF, DOC, DOCX, JPG, PNG, GIF, WebP, TIFF, BMP, TXT, RTF, CSV, XLS, XLSX, DICOM</span>
          </div>
